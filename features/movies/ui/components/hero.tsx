...
export const HeroSection = ({
  ...
  return (
    <>
      <section
        className={cn(
          "relative overflow-hidden rounded-b-[20px]",
          searchQuery.length > 0
            ? "h-[80px]"
            : "h-[500px] sm:h-[600px] md:h-[680px]",
        )}
      >
        <motion.img
          initial={{ scale: 1.08 }}
        ...
        <div
          className={cn(
            "absolute right-4 md:right-1/2",
            searchQuery.length > 0
              ? "fixed top-20 h-auto z-50 md:w-full left-4 md:left-10"
              : "top-16 sm:top-20 h-full z-20 left-4 md:left-24",
          )}
        >
          <motion.div
            ...
            className={cn(
              "flex items-center gap-2 bg-white/5 backdrop-blur border border-white/10 rounded-full px-4 py-2 max-w-full md:max-w-md mb-8 md:mb-16 focus-within:border-primary/50 transition-colors",
            )}
          >
            ...
            <input
              ...
              className="bg-transparent text-sm outline-none flex-1 placeholder:text-white/40 text-white"
            />
          </motion.div>
          ...
          {searchQuery.length === 0 && (
            <>
              ...
              >
                {/* Direct Watch Link to the actual Star Wars movie ID on TMDB (Star Wars EP IX is 181812) */}
                <motion.a
                  href="/movie/181812"
                  whileHover={{
                    scale: 1.05,
                  ...
                  whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2 bg-primary rounded-full px-6 py-2.5 text-sm font-medium text-white"
                >
                ...
                <motion.a
                  ...
                  whileTap={{ scale: 0.96 }}
                  className="rounded-full px-6 py-2.5 text-sm border border-white/40 text-white flex items-center justify-center"
                >
                  Details
                </motion.a>
              </motion.div>
      ...
      {searchQuery.length === 0 && <PopularPanel />}
    </>
  );
};
